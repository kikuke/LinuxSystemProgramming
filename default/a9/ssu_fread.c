#include <stdio.h>
#include <stdlib.h>

struct ssu_pirate {
    unsigned long booty;
    unsigned int beard_length;
    char name[128];
};

int main(void)
{
    struct ssu_pirate blackbeard = {950, 48, "Edward Teach"}, pirate;
    char *fname = "ssu_data";
    FILE *fp1, *fp2;

    //ssu_data라는 파일을 쓰기로 새로 오픈
    if ((fp2 = fopen(fname, "w")) == NULL) {
        fprintf(stderr, "fopen error for %s\n", fname);
        exit(1);
    }

    //blackbeard라는 구조체를 ssu_pirate구조체 1개 크기만큼 fp2가 오픈한 파일에 바이너리로 쓰기
    if (fwrite(&blackbeard, sizeof(struct ssu_pirate), 1, fp2) != 1) {
        fprintf(stderr, "fwrite error\n");
        exit(1);
    }

    if (fclose(fp2)) {
        fprintf(stderr, "fclose error\n");
        exit(1);
    }

    //위에서 썼던 파일을 읽기모드로 다시오픈함
    if ((fp1 = fopen(fname, "r")) == NULL) {
        fprintf(stderr, "fopen error\n");
        exit(1);
    }

    //오픈한 파일을 ssu_pirate구조체 1개 크기만큼 읽어서 pirate에 저장함
    if (fread(&pirate, sizeof(struct ssu_pirate), 1, fp1) != 1) {
        fprintf(stderr, "fread error\n");
        exit(1);
    }

    if (fclose(fp1)) {
        fprintf(stderr, "fclose error\n");
        exit(1);
    }

    //읽어들인 데이터를 출력
    printf("name=\"%s\" booty=%lu beard_length=%u\n",
        pirate.name, pirate.booty, pirate.beard_length);
    exit(0);
}